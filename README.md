...
4. **Pro:** همگام‌سازی ابری لیست سرور، قواعد split-bypass، آمار دقیق‌تر per-process (نیاز به APIهای دیگر غیر از WinINet).
5. **آیکون اختصاصی:** یک `app.ico` بسازید و در `resources/app.rc` با `IDI_APPICON` وصل کنید.
